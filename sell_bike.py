...
# Defining a function to sell bikes from the library that accepts the following 2D list as the parameter
def sell_bike(listed_items):
    ...
    print()
    # Asking the user to input their first and last names and checking through the regular expression if correct values are entered
    ...
    while True:
        ...
        if not (re.match("^[a-zA-Z]*$", first_name) and re.match("^[a-zA-Z]*$", last_name)):
            ...
            print("Greetings " + string_name + ",do follow the instructions to buy a bike. ")
            break

    ordering_bike = True
    while ordering_bike:
        print()
        bike_id = (input("Enter the ID of the bike you wish to buy: ")).upper()
        ...
        if bike_id == list_data[0][0] or bike_id == list_data[1][0] or bike_id == list_data[2][0] or \
            ...
            # Substracting the quantity of bike as the value and bike code as its key on the dictionary initialized
            if bike_id == list_data[0][0] and bike_quantity < int(list_data[0][4]):
                dictionary_data[bike_id] = bike_quantity
            elif bike_id == list_data[1][0] and bike_quantity < int(list_data[1][4]):
                dictionary_data[bike_id] = bike_quantity
            elif bike_id == list_data[2][0] and bike_quantity < int(list_data[2][4]):
                dictionary_data[bike_id] = bike_quantity
            elif bike_id == list_data[3][0] and bike_quantity < int(list_data[3][4]):
                dictionary_data[bike_id] = bike_quantity
            elif bike_id == list_data[4][0] and bike_quantity < int(list_data[4][4]):
                dictionary_data[bike_id] = bike_quantity
            elif bike_id == list_data[5][0] and bike_quantity < int(list_data[5][4]):
                dictionary_data[bike_id] = bike_quantity
            elif bike_id == list_data[6][0] and bike_quantity < int(list_data[6][4]):
                dictionary_data[bike_id] = bike_quantity
            elif bike_id == list_data[7][0] and bike_quantity < int(list_data[7][4]):
                dictionary_data[bike_id] = bike_quantity
            elif bike_id == list_data[8][0] and bike_quantity < int(list_data[8][4]):
                dictionary_data[bike_id] = bike_quantity
            elif bike_id == list_data[9][0] and bike_quantity < int(list_data[9][4]):
                dictionary_data[bike_id] = bike_quantity
            # Error message when the quantity inputted is less than the bikes in stock
            else:
        ...
        while True:
            print()
            supplement = input("Do you wish to buy any more bikes (Y/N)? ").upper()
    ...
    # Creation of selling notice invoice for the selling bikes
    ...
    file_write.write("\n")
    file_write.write("\n")
    file_write.write(str("\t\tName of the Company: " + str(string_name) + "\t\t\t" + "Date: " + str(
        date) + "\t\t\t\t " + "Time: " + str(time)))
    file_write.write("\n")
    file_write.write(
        "----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------")
    ...
    file_write.write("\n")
    file_write.close()
    # It returns the dictionary that stored the details of bike code and bike sold
